...
gst-launch-1.0 \
    ...
    queue leaky=2 max-size-buffers=2 ! \
    tensor_decoder mode=bounding_boxes option1=yolov8 option2=labels.txt option4=640:640 option5=320:320 ! \
    videoscale ! videoconvert ! video/x-raw,width=640,height=640,format=RGBA ! \
    compositor.sink_1 \
    t. ! queue leaky=2 max-size-buffers=2 ! \
    videoconvert ! video/x-raw,width=640,height=640,format=RGBA ! \
    compositor.sink_0 \
